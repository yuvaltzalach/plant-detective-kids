...
export function BotiMascot() {
  ...
  return (
    <>
      {/* בּוֹטִי — קבוע בצד שמאל למעלה, בלי ריחוף (רק אנימציות מתוזמנות בדמות עצמה) */}
      <div className="fixed left-2 top-16 z-50 flex items-start gap-2">
        <button
          onClick={() => {
            playPop();
            setBubbleOpen((b) => !b);
          }}
          className="drop-shadow-xl active:scale-90"
        ...
        {bubbleOpen && (
          <div className="relative mt-2 max-w-[190px] rounded-2xl rounded-tl-none bg-white p-3 pt-6 shadow-xl">
            <button
              onClick={() => setBubbleOpen(false)}
              className="absolute right-1 top-1 flex h-5 w-5 items-center justify-center rounded-full bg-purple-200 text-xs font-bold text-purple-700"
              aria-label="סגירה"
            >
              ✕
            </button>
