...
export async function enrichToResult(candidate: IdentifyCandidate): Promise<PlantResult> {
  ...
  // ננסה ויקיפדיה עברית לפי השמות הנפוצים ואז לפי השם המדעי
  const titles = [...(candidate.commonNames ?? []), candidate.scientificName].filter(
    Boolean
  );
  ...
  // גיבוי אחרון — טקסט כללי ידידותי
  const friendlyName = candidate.commonNames[0] || candidate.scientificName || "צמח מסתורי";
  return {
