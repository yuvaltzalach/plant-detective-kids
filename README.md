...
- 🔐 **חשבון אמיתי (שם משתמש + סיסמה)** — נרשמים פעם אחת (שם משתמש, גיל, סיסמה) ומכאן כניסה אוטומטית מאותו טלפון, עד שמתנתקים. אפשר להתחבר לאותו חשבון מכל מכשיר וההתקדמות מסתנכרנת (דורש Cloudflare KV).
- 🧒/👨‍👩‍👧 **גיל קובע** — עד גיל 16 חשבון ילד/ה (בלי אזור הורים); מעל 16 חשבון הורה עם אזור ניהול. גם להורה יש מסך משחק רגיל.
- 👀 **אזור הורים** — מזינים שם משתמש + סיסמה של הילד/ה כדי לקשר את החשבון שלו/ה, ואז רואים את ההתקדמות (נקודות, רמה, אלבום, תגים) מכל מכשיר.
- 🌱 **בּוֹטִי** — דמות מנחה שמעודדת ומדברת (הקראה בעברית).
- 🏅 **תגים עונתיים** — פריחת אביב, חוקר קיץ, שלכת סתיו וחוקר חורף.
...
## 🔊 שדרוג אופציונלי: קול ילד אמיתי (ElevenLabs TTS)
ברירת המחדל היא קול המכשיר (Web Speech, גובה גבוה כדי להישמע צעיר). לקול ילד אמיתי
ואיכותי בעברית אפשר להפעיל ElevenLabs:
```
ELEVENLABS_API_KEY=...
ELEVENLABS_VOICE_ID=<voice id של קול ילדי מהספרייה שלכם>
TTS_ENABLED=true
```
פונקציית `/api/tts` ממירה טקסט ל-MP3 בצד השרת (המפתח לא נחשף) וממטמנת ב-KV לפי הטקסט
כדי לחסוך בעלות. בלי הגדרה — נשאר קול המכשיר.
